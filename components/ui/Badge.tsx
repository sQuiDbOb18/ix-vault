...
export function Badge({ status }: { status: PaymentStatus }) {
  ...
  return (
    <span className={cn("status-badge inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-medium", map[status])}>
      <span className={cn("status-dot h-1.5 w-1.5 rounded-full bg-current", status !== "Paid" && "animate-[pulse_2s_ease-in-out_infinite]")} />
      {status}
    </span>
  );
}
