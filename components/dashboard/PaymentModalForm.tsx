...
import { AlertCircle, CloudUpload, FileImage, FileText, Pencil, X } from "lucide-react";
...
function MemberNameField({ form, members, locked = false, onUnlock }: { form: UseFormReturn<PaymentInput>; members: Member[]; locked?: boolean; onUnlock?: () => void }) {
  return (
    <Controller
      ...
      render={({ field, fieldState }) => (
        <label className="modal-field-group block">
          <span className="mb-2 flex items-center justify-between gap-3">
            ...
            {locked && (
              <button
                type="button"
                onClick={onUnlock}
                className="inline-flex items-center gap-1 text-xs font-medium text-text-accent transition hover:text-text-primary"
              >
                <Pencil size={12} />
                Change
              </button>
          ...
          <input
            {...field}
            list={locked ? undefined : "member-names"}
            autoComplete="off"
            readOnly={locked}
            aria-readonly={locked}
            className={cn(inputClass(Boolean(fieldState.error)), locked && "cursor-not-allowed bg-[var(--bg-elevated)] text-text-secondary opacity-75")}
          />
          {!locked && <datalist id="member-names">{members.map((member) => <option key={member.id} value={member.name} />)}</datalist>}
...
export function PaymentModalForm({
  ...
  members?: Member[];
  memberNameLocked?: boolean;
  file: File | null;
  currentReceiptUrl?: string | null;
  uploadError?: string | null;
  isSubmitting: boolean;
  submitLabel: string;
  onUnlockMemberName?: () => void;
  onFile: (file?: File) => void;
  onRemoveFile: () => void;
  onCancel: () => void;
  onSubmit: () => void;
    ...
    <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="space-y-5">
      <section className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-5 shadow-[0_1px_0_rgba(255,255,255,0.04)_inset]">
        ...
        <div className="space-y-4">
          <MemberNameField form={form} members={members} locked={memberNameLocked} onUnlock={onUnlockMemberName} />
